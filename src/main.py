...
def run_simulation(simulation_time: int, pop_size: int, urgency_scenario: int, history_length_personal: int, history_length_reported: int, exploration_rate: float, system_optimum: int, nash_equilibrium: int, simulation_type : int, history_weight_personal:float, history_weight_reported:float,  config) -> None:
    ...
    memory_route_A = [deque(maxlen=history_length_personal) for _ in range(len(population))]
    memory_route_B = [deque(maxlen=history_length_personal) for _ in range(len(population))]
    history_reported_A = deque(maxlen=history_length_reported)
    ...
    for time in range(0, simulation_time):
        ...
        records.append({
            ...
            "flow_A": flow_A,
            "flow_B": flow_B,
            "mean_traveltime_A": mean_traveltime_A,
            "mean_traveltime_B": mean_traveltime_B
        })
    ...
    # Create dataframe (done so late, because lists are more efficient than dataframe)
